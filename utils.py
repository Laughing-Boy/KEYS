...
class RetrievalQAEmbedder(torch.nn.Module):
    ...
    def embed_sentences_checkpointed(self, input_ids, attention_mask, checkpoint_batch_size=-1):
        ...
        if checkpoint_batch_size < 0 or input_ids.shape[0] < checkpoint_batch_size:
            ...
            extended_attention_mask = self.sent_encoder.get_extended_attention_mask(
...
def qa_s2s_generate(
    ...
    # print(generated_ids);assert False
    return ([qa_s2s_tokenizer.decode(ans_ids, skip_special_tokens=True).strip() for ans_ids in generated_ids.sequences],generated_ids)
...
def batch_query_qa_dense_index_nn(passages, qa_embedder, tokenizer, wiki_passages, wiki_index, n_results=10):
    ...
    for (res_passages, dl, il) in zip(res_passages_lst, D, I):
        ...
        for r, sc, i in zip(res_list, dl, il):
            ...
            r["score"] = float(sc)
        all_res_lists += [res_list[:]]
    return support_doc_lst, all_res_listsy
